...
use eframe::egui;
use egui::{ColorImage, Rect, Scene, TextureHandle};
use image::RgbImage;
...
impl GameViewer {
    ...
    fn update_board_texture(&mut self, ctx: &egui::Context) {
        if let Ok(rgb_image) = self.game.generate_board_image(self.scale, None, false) {
            ...
            self.board_texture = Some(ctx.load_texture("board", color_image, texture_options));
        }
    }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::left("control_panel").show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                ui.label(format!(
                    "Iteration: {}/{}",
                    self.game.iteration, self.game.max_iterations
                ));
                ...
                if ui.button("💾 Save Image").clicked() {
                    ...
                    if let Err(e) =
                        self.game
                            .generate_board_image(self.scale, Some(&filename), true)
                    {
                ...
                egui::CollapsingHeader::new(format!("Teams ({})", self.game.teams.len()))
                    ...
                    .show(ui, |ui| {
                        ...
                        for (team_id, cells) in sorted_teams {
                            ...
                            .show(ui, |ui| {
                                // Team statistics
                                let total_energy: u32 =
                                    cells.iter().map(|(_, cell)| cell.energy).sum();
                                let avg_energy = if !cells.is_empty() {
                                    total_energy / cells.len() as u32
                                } else {
                                    0
                                };
                                let avg_age: f32 = if !cells.is_empty() {
                                    cells.iter().map(|(_, cell)| cell.age as f32).sum::<f32>()
                                        / cells.len() as f32
                                } else {
                                    0.0
                                };
                                        ...
                                        egui::Grid::new(format!("team_{}_cells", team_id.0))
                                            ...
                                            .show(ui, |ui| {
                                                ...
                                                for (cell_id, cell) in sorted_cells {
                                                    ...
                                                    if let Some(pos) =
                                                        self.game.inv_coordinate_map.get(cell_id)
                                                    {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            egui::Frame::group(ui.style())
                .inner_margin(0.0)
                .show(ui, |ui| {
                    let scene = Scene::new().zoom_range(0.1..=10.0); // Allow zooming from 10% to 1000%
