...
pub fn generate_energy(
    ...
    let scattered_energy_values =
        sample_from_distribution(&config.scattered_energy, seed, config.num_scattered);
    let plant_rate_values = sample_from_distribution(&config.plant_rate, seed, config.num_plants);
    let plant_max_energy_values =
        sample_from_distribution(&config.plant_max_energy, seed, config.num_plants);
