...
use crate::{
    config::{CellConfig, FileConfig, GameConfig},
    game::Game,
    world_gen::{generate_energy, generate_terrain},
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config_path_to_load = cli_args.config_path.clone().unwrap_or_else(|| {
        println!(
            "No config path specified via CLI, trying default: {:?}",
            default_config_path
        );
        default_config_path
    });
    ...
    let file_config: FileConfig = if config_path_to_load.exists() {
        ...
        let config_str = fs::read_to_string(&config_path_to_load).map_err(|e| {
            format!(
                "Failed to read config file {:?}: {}",
                config_path_to_load, e
            )
        })?;
        ...
        if cli_args.config_path.is_some() {
            ...
            eprintln!(
                "Error: Config file specified at {:?} not found.",
                config_path_to_load
            );
            ...
            eprintln!(
                "Warning: Default config file not found at {:?}, using internal defaults.",
                config_path_to_load
            );
            FileConfig::default()
        }
    ...
    let game_config = GameConfig {
        ...
        terrain_type: cli_args
            .terrain_type
            .unwrap_or(file_config.world.terrain_type),
        ...
        cell_config: CellConfig {
            // Ensure all fields from file_config.cell are used
    ...
    println!(
        "Terrain type: '{}', Levels: {}",
        game_config.terrain_type, game_config.terrain_levels
    );
    println!("Max iterations: {}", game_config.max_iterations);
    println!("Energy config: {:?}", game_config.energy_options);
    println!(
        "Cell config: Min Energy: {}, Initial Energy: {}, Starting Cells/Team: {}, Max Energy: {}, Min Attack: {}, Max Attack: {}, Attack Scaling Energy: {}",
    ...
    let terrain_vec = match game_config.terrain_type.as_str() {
        ...
        "noise" => generate_terrain(
            game_config.width,
            game_config.height,
            game_config.seed,
            game_config.terrain_levels,
        ),
        other => {
    ...
    let energy_vec = generate_energy(
        game_config.width,
        game_config.height,
        game_config.seed,
        &game_config.energy_options,
    );
    ...
    let mut game = if let Some(state_file) = &cli_args.load_state {
        ...
        println!(
            "Loaded state from iteration {} (max: {})",
            state.iteration, state.max_iterations
        );

        // Create team paths from CLI args
        let team_paths: Vec<(TeamId, PathBuf)> = game_config
            .mind_paths
            .iter()
            .enumerate()
    ...
    if cli_args.gui {
        ...
        println!(
            "Executed {} steps. Final iteration: {}/{}",
            executed, game.iteration, game.max_iterations
        );
