...
use crate::action_capnp::{action, Direction as CapnpDirection};
use capnp::serialize;
...
pub fn cell_action_to_capnp(cell_action: &CellAction) -> capnp::Result<Vec<u8>> {
    let mut message = ::capnp::message::Builder::new_default();
    let mut action_builder = message.init_root::<action::Builder>();
    ...
    match cell_action {
        ...
        CellAction::Split(dir, energy, marker, memory) => {
...
pub fn capnp_to_cell_action(data: &[u8]) -> capnp::Result<CellAction> {
    let message_reader = serialize::read_message(&mut data.as_ref(), ::capnp::message::ReaderOptions::new())?;
    let action_reader = message_reader.get_root::<action::Reader>()?;
    ...
    let cell_action = match action_reader.which()? {
        ...
        action::Which::Split(reader) => {
            ...
            memory.copy_from_slice(memory_reader);
            CellAction::Split(dir, energy, marker, memory)
        }
        action::Which::Eat(()) => {
            CellAction::Eat
